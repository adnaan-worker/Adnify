...
## [1.7.18] - 2026-03-21 🧠 混合 Agent IDE 第一阶段
...
### ✨ 优化
- ⚡ **渲染卡顿优化** (adnaan)
  - 聚焦界面渲染延迟与交互卡顿问题，优化多处显示链路，减少高频操作时的阻塞感。
- 🩹 **稳定性修复** (adnaan)
  - 修复一组已知问题，提升日常编辑、任务执行与面板切换时的整体稳定性。

---

## [1.7.16] - 2026-03-19 🧰 参数校验与终端稳定性增强

> "把调用细节打磨扎实，执行链路就会更可靠" 🔧

### ✨ 新功能 / 优化
- 🧪 **调用参数校验优化** (adnaan)
  - 优化工具调用参数的校验逻辑，提升执行准确性，减少因参数不匹配导致的失败。

### 🐛 Bug 修复
- 🍎 **macOS Agent 终端稳定性修复** (adnaan)
  - 修复 macOS 下 Agent 终端 PTY 的崩溃问题，改善交互式执行体验。
- 🔀 **aisuper 分支改动合入** (adnaan, joanboss)
  - 合入 `dev-aisuper` 分支的改进，补齐相关稳定性与执行细节优化。
